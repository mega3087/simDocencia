					...
					<div class="col-lg-12">
						<div class="panel panel-primary">
							...
							<div class="panel-body">
								<?php
									echo form_open_multipart("archivo/index/$nombre_campo/$tipo_campo", 'id="upload" class="upload"');
									$tipo_campo_a = $tipo_campo;
									if ( strrpos($tipo_campo_a,"pdf")!== false ){
										$tipo_campo_a = str_replace("pdf",'application/pdf',$tipo_campo_a);
										$tipo_campo_a = str_replace("_",', image/',$tipo_campo_a);
									}else{
										$tipo_campo_a = "image/".str_replace("_",',image/',$tipo_campo_a);
									}
									echo '<label title="Subir archivo" for="userfile" class="btn btn-primary">
									<input type="file" accept="'.$tipo_campo_a.'" name="userfile" id="userfile" class="hide">
									Seleccionar archivo
									</label>';
									echo '<label><input class="disabled" disabled id="archivo_nombre" size="38%"></label>'. br('2');
									
									echo "<input type='submit' class='button btn btn-primary hidden' id='submit' value='Subir archivo'>" . nbs(5);
									echo '<span id="loader" style="display:none;">';
									echo img('assets/img/loader.gif') . nbs() . 'Subiendo archivo, espere por favor';
									echo '</span>';
									echo br(2);
									...
									if (isset($datos_archivo['file_name'])) {
										echo '<div><strong>Archivo subido:</strong>'.nbs(2);
										echo '<span id="archivo_subido">';
										echo anchor(base_url($datos_archivo['file_name']), "Ver archivo" , 'class="iconiza" TARGET=_blank');
										echo form_hidden('file_name', $datos_archivo['file_name'] );
										echo '</span></div>' . br();
									}
									$tipo_campo = strtoupper( str_replace("_",'|',$tipo_campo) );
									echo "<em>Sólo se permiten archivos $tipo_campo con tamaño máximo de 2MB.</em>";
									echo form_close();
								?>		
							</div>
						</div>
