...
class Archivo_csv extends CI_Controller 
{
    ...
    function index($nombre_campo) 
    {
        ...
        $this->load->view('archivo_view_csv', $data);
		
		$file = @$data ['datos_archivo'] ['file_name'];

		if($file){
			$data = $this->csv->parse_file($file);
			foreach($data as $key => $list){
				$list['PRUsuario_registro'] = get_session('UNCI_usuario');
				$this->producto_model->insert($list);
			}
		}
